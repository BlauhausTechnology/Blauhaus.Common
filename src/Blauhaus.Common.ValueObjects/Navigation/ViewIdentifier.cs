...
namespace Blauhaus.Common.ValueObjects.Navigation;
...
public class ViewIdentifier : BaseValueObject<ViewIdentifier>, IHasName, IHasProperties
{
    ...
    public static ViewIdentifier Deserialize(string serialized)
    {
        ...
        if (split.Length > 1)
        {
            ...
            string[] queryStringParameters = queryString.Split('&');
            foreach (string queryStringParameter in queryStringParameters)
            {
                string[] kvp = queryStringParameter.Split('=');
